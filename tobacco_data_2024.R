...
data_tobacco <- read_delim("TobaccoRegister_02-19-2024.csv", delim = "|")
#NRS SPD data ('Small User' CSV; postcodes -> datazone 2011 code; deleted entries removed)
data_nrs_spd <- read_csv("SmallUser_2024_deleted_removed.csv")
...
data_tobacco %>%
  ...
  inner_join(data_nrs_spd[c('Postcode', 'DataZone2011Code')], by = 'Postcode') %>%
  #10935 outlets
...
data_tobacco_transformed %>%
  ...
  inner_join(data_nrs_sap[c('Data zone code', 'Total population')], by = c('DataZone2011Code' = 'Data zone code')) %>%
  ...
  #calculate outlet densities of each datazone, standardised to outlets per 1000 people (all types, and two custom sub-types), add new columns
  mutate (all_density = (dz_count/total_pop)*1000) %>%
  mutate (tobacco_density = ((t_and_nvp_count + t_only_count)/total_pop)*1000) %>%
  mutate (nvp_density = ((t_and_nvp_count + nvp_only_count)/total_pop)*1000) -> data_tobacco_transformed_densities

#final transformed data
data_tobacco_transformed_densities %>%
  distinct(DataZone2011Code, dz_count, t_and_nvp_count, nvp_only_count, t_only_count, total_pop, all_density, tobacco_density, nvp_density, .keep_all = FALSE) -> data_tobacco_transformed_final
  #3768 final rows in total, as many of the 6676 datazones have 0 retail outlets

## Export data
write.csv(data_tobacco_transformed_final, "smoking_data_transformed.csv")